...
window.sfBlazor.Diagram = function () {
  ...
  var diagram = {
    ...
    createMeasureElements: function createMeasureElements(isZoomValue, layerList, width, height, transform, patternWidth, patternHeight, elementId, component, gridLinePathData, dotsGrid) {
      this.updateZoomPanTool(isZoomValue);
      this.updateInnerLayerSize(layerList, width, height, undefined, undefined, undefined, transform, patternWidth, patternHeight, gridLinePathData, dotsGrid);
    ...
    updateGridlines: function updateGridlines(patternId, patternWidth, patternHeight, gridLinePathData, dotsGrid) {
      var layer = document.getElementById(patternId);

      if (layer) {
        layer.setAttribute("width", patternWidth.toString());
        layer.setAttribute("height", patternHeight.toString());
      ...
      updateInnerLayerSize: function updateInnerLayerSize(layerList, width, height, left, top, eventValue, transform, patternWidth, patternHeight, gridLinePathData, dotsGrid, lineAttributes, pathAttributes, rectAttributes, circleAttributes) {
        ...
        for (var i = 0; i < layerList.length - 1; i++) {
          ...
          if (i >= layerList.length - 4) {
              if (layerList[i].indexOf("_pattern") != -1) {
                  this.updateGridlines(layerList[i], patternWidth, patternHeight, gridLinePathData, dotsGrid);
              //layer.setAttribute("width", patternWidth.toString());
              //layer.setAttribute("height", patternHeight.toString());

              //if (layer.children.length > 0) {
              //  for (var k = 0; k < layer.children.length; k++) {
              //    if (gridLinePathData && gridLinePathData.length > 0) {
              //      layer.children[k].setAttribute("d", gridLinePathData[k]);
              //    } else if (dotGridCx && dotGridCy) {
              //      layer.children[k].setAttribute("cx", dotGridCx[k].toString());
              //      layer.children[k].setAttribute("cy", dotGridCy[k].toString());
              //    }
              //  }
              //}
              ...
              var selectorlayer = document.getElementById(layerList[i].split("_")[0] + "_SelectorElement"); //selectorlayer.setAttribute("transform", gTransform);
              ...
              if (selectorlayer.children.length > 0) {
                ...
                selectorlayer.children[0].setAttribute("x1", x1.toString());
                selectorlayer.children[0].setAttribute("y1", y1.toString());
                selectorlayer.children[0].setAttribute("x2", x2.toString());
                selectorlayer.children[0].setAttribute("y2", y2.toString());
                selectorlayer.children[0].setAttribute("transform", "rotate(" + lineAttributes.angle + " " + (lineAttributes.x + lineAttributes.width * lineAttributes.pivotX) + " " + (lineAttributes.y + lineAttributes.height * lineAttributes.pivotY) + ")"); //string transform = "rotate(" + attributes.Angle + "," + (attributes.X + attributes.Width * attributes.PivotX) + "," + (attributes.Y + attributes.Height * attributes.PivotY) + ")" + "translate(" + attributes.X + "," + attributes.Y + ")";

                selectorlayer.children[1].setAttribute("transform", "rotate(" + pathAttributes.angle + "," + (pathAttributes.x + pathAttributes.width * pathAttributes.pivotX) + "," + (pathAttributes.y + pathAttributes.height * pathAttributes.pivotY) + ")" + "translate(" + pathAttributes.x + "," + pathAttributes.y + ")");
                selectorlayer.children[2].setAttribute("x", rectAttributes.x);
                selectorlayer.children[2].setAttribute("y", rectAttributes.y);
                selectorlayer.children[2].setAttribute("width", rectAttributes.width);
                selectorlayer.children[2].setAttribute("height", rectAttributes.height);
                selectorlayer.children[2].setAttribute("transform", "rotate(" + rectAttributes.angle + "," + (rectAttributes.x + rectAttributes.width * rectAttributes.pivotX) + "," + (rectAttributes.y + rectAttributes.height * rectAttributes.pivotY) + ")");
                var circleAttributesCount = 0;

                for (var k = 3; k < selectorlayer.children.length; k++) {
                  selectorlayer.children[k].setAttribute("cx", circleAttributes[circleAttributesCount].cx);
                  selectorlayer.children[k].setAttribute("cy", circleAttributes[circleAttributesCount].cy);
    ...
    onAddWireEvents: function onAddWireEvents(id, component) {
      ...
      window.addEventListener('resize', function (e) {
        _this.invokeDiagramEvents(e, component, id);
      });
    ...
    invokeDiagramEvents: function invokeDiagramEvents(e, component, id, paletteDragAndDrop) {
      ...
      if (!editTextBoxDiv && ((e.type == "mousemove" || e.type == "touchmove") && !eventStarted || e.type != "mousemove" || e.type != "touchmove" || !inAction)) {
        //if ((((e.type == "mousemove" || e.type == "touchmove") && !eventStarted) || (e.type != "mousemove" && e.type != "touchmove") || !inAction)) {
        if ((e.type == "mousemove" || e.type == "touchmove") && action && isZoomPan) {
          eventStarted = true;
