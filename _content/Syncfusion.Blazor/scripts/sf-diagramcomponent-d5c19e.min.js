...
window.sfBlazor.Diagram = function () {
    ...
    function InitDraggable(parent, symbolPaletteInstance) {
      ...
      this.drop = function (e) {
        ...
        if (ParentElement && e.event.changedTouches && e.event.changedTouches.length > 0) {
          var diagramcomponent = void 0;

          for (var i = _this.symbolPaletteInstance.length - 1; i >= 0; i--) {
            diagramcomponent = _this.symbolPaletteInstance[i].componentInstance;
            break;
          }

          if (diagramcomponent) {
            window["sfBlazor"].Diagram.invokeDiagramEvents(e.event, diagramcomponent, undefined, e);
          }
  ...
  var diagram = {
    ...
    setAttribute: function setAttribute(element, attributes) {
      ...
      createMeasureElements: function createMeasureElements(isZoomValue, layerList, width, height, transform, patternWidth, patternHeight, elementId, component, gridLinePathData) {
      this.updateZoomPanTool(isZoomValue);
        this.updateInnerLayerSize(layerList, width, height, undefined, undefined, undefined, undefined, transform, patternWidth, patternHeight, gridLinePathData);
    ...
    updateZoomPanTool: function updateZoomPanTool(val) {
      ...
      updateInnerLayerSize: function updateInnerLayerSize(layerList, width, height, left, top, eventValue, id, transform, patternWidth, patternHeight, gridLinePathData) {
      if (layerList != undefined && width != undefined && height != undefined && layerList.length > 0) {
        var layer = void 0;
          console.log("layer width:" + width);
          console.log("layer height:" + height);
          console.log("layer left:" + left);
          console.log("layer top:" + top);
        ...
        for (var i = 0; i < layerList.length - 1; i++) {
          layer = document.getElementById(layerList[i]);
            ...
            if (i >= layerList.length - 4) {
                if (layerList[i].indexOf("_pattern") != -1) {
                    layer.setAttribute("width", patternWidth);
                    layer.setAttribute("height", patternHeight);
                    //if (gridLinePathData && layer.children.length > 0) {
                    ...
                    layer.children[0].setAttribute("x", x);
                    layer.children[0].setAttribute("y", y);
                }
          ...
          //layer = document.getElementById(layerList[layerList.length - 1]);
